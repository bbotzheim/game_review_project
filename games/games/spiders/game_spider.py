...
class GameSpider(Spider):
    ...
    def parse_game_page(self, response):
        ...
        item['average_user_score'] = float(response.xpath('//*[@id="main_content"]//div[@class="details side_details"]//a[@class="metascore_anchor"]/div/text()').extract_first()) #float
        item['metascore'] = float(response.xpath('//*[@id="main_content"]//div[@class="details main_details"]//a[@class="metascore_anchor"]/div/span/text()').extract_first()) #float
    ...
    def parse_user_reviews(self, response):
        ...
        for review in reviews:
            ...
            score = review.xpath('.//div[@class="metascore_w user medium game negative indiv"]/text()').extract_first() # int
        ...
        print('Finished Scraping ' + len(reviews) + 'User Reviews for ' + title)
    ...
    def parse_critic_urls(self, response):
        ...
        user_urls = [response.meta['url'] + f'/user-reviews?page={i}' for i in range(0, num__critic_pages)]
    ...
    def parse_critic_reviews(self, response):
        ...
        for review in reviews:
            username = review.xpath('.//div[@class="source"]/a/text()').extract_first().strip() #extra white space?
            score = review.xpath('.//div[@class="metascore_w medium game positive indiv perfect"]/text()').extract_first()
            try:
                score = int(score)
            except Exception:
                pass
            ...
            yield item
        print('Finished Scraping ' + len(reviews) + 'Critic Reviews for ' + title)
